$(function () {
    ...
    if (window.location.href.indexOf("/widget/") > 0) return;
    ...
    window.addEventListener('beforeunload', function () { (<any>window).ShowPleaseWait() });
    ...
    setInterval(() => {
        if (form.get(0).scrollHeight > form.height()) syncHubFrame();
    }, 5000);
});
...
function measureWidth(item): number {
...
function notifyHub(command: string, arg) {
