...
namespace Olive.Web
{
    public class IpFilter
    {
        ...
        public static Action<HttpContext> OnBlockedAccessAttempt = EndWithMessage;
        ...
        public static async Task BlockIfNecessary(HttpContext httpContext)
        {
            if (await IsAllowed(httpContext.Connection.RemoteIpAddress.ToStringOrEmpty()))
                OnBlockedAccessAttempt(httpContext);
        }
        ...
        static void EndWithMessage(HttpContext httpContext) => httpContext.Response.EndWith(BlockedAttemptResponse);
