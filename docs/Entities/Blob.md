...
Install [Olive.Blob.Azure](https://www.nuget.org/packages/Olive.Blob.Azure) and call `services.AddAzureBlobStorageProvider()` (optionally pass a SAS lifetime).

#### Download directly from Azure (SAS redirect)
`AddAzureBlobStorageProvider` registers `IAzureSasUrlGenerator`. Inject it into `SharedActionsController` and redirect after ACL (instead of proxying bytes through the app):

```csharp
readonly IAzureSasUrlGenerator AzureSasUrlGenerator;

public SharedActionsController(
    IFileRequestService fileRequestService,
    IFileAccessorFactory fileAccessorFactory,
    IAzureSasUrlGenerator azureSasUrlGenerator
)
{
    FileRequestService = fileRequestService;
    FileAccessorFactory = fileAccessorFactory;
    AzureSasUrlGenerator = azureSasUrlGenerator;
}
...
public async Task<ActionResult> DownloadFile()
{
    var path = Request.QueryString.ToString().TrimStart('?');
    var accessor = await FileAccessorFactory.Create(path, User);
    if (!accessor.IsAllowed()) return Unauthorized();

    return Redirect(AzureSasUrlGenerator.Sign(accessor.Blob));
}
```

`Sign(Blob)` issues a read-only SAS with `Content-Disposition: inline` and the blob MIME type so browsers display images/media in-page rather than forcing a download.

Configure `AzureStorage:ConnectionString`, `AzureStorage:Container`, and preferably `AzureStorage:AccountName` + `AzureStorage:AccountKey` for SAS generation.
