...
namespace Olive
{
    ...
    {
        public static Task<List<T>> ToListAsync<T>(this IEnumerable<Task<T>> list) => list.AwaitAll().ToList();

        public static Task<T[]> ToArrayAsync<T>(this IEnumerable<Task<T>> list) => list.AwaitAll().ToArray();
        ...
        {
            return list.Select(func).AwaitAll();
        }
        ...
        {
            return (await list.SelectAsync(func)).SelectMany(x => x);
        }
        ...
        {
            return list.SelectMany(func).AwaitAll();
        }
    }
}