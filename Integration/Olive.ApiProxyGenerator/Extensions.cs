...
namespace Olive.ApiProxy
{
    static class Extensions
    {
        ...
        {
            var duplicateRemoteDataProviders = from method in methods
                                               let returnType = method.GetAttribute("Returns")?.ConstructorArguments.Single().Value as Type
                                               let markedAsRemoteDataProvider = method.GetCustomAttribute<RemoteDataProvider>() != null
            ...
            if (duplicateRemoteDataProviders.Any())
            {
                var duplicateErrorMessage = (from g in duplicateRemoteDataProviders
                                             let type = g.Key
