        ...
        internal static string EncodeMatches(string html, string[] patterns)
        {
            ...
            foreach (var pattern in patterns)
            {
                ...
                catch (RegexMatchTimeoutException)
                {
                    ...
                    // Warning, not Error: nothing is broken, but a pattern that keeps timing out
                    // is doing no work at all, so it needs to be seen and rewritten.
                    try
                    {
                        Log.For(typeof(HtmlSanitizerFactory)).Warning(
                            $"[HtmlSanitizer] EncodePatterns entry timed out after {PatternTimeout.TotalMilliseconds}ms and was skipped." +
                            $"\nPattern: {pattern}" +
                            $"\nInput ({html.Length} chars): {html.Left(500)}");
                    }
